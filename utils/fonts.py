...
def attach_font_fgcolor(font: Any, fgcolor: Tuple[int, int, int]) -> Any:
...
def _sysfont_chinese(size: int, fgcolor: Tuple[int, int, int]) -> "Optional[pygame.font.Font]":
...
def load_font_chinese(
    size: int,
    fgcolor: Tuple[int, int, int],
    *,
    weight: str = "regular",
) -> "Optional[pygame.font.Font]":
    """중국어용 폰트 (문장·병음). `fgcolor`는 (R,G,B) 튜플로만 넘긴다."""
    from core.paths import DEFAULT_FONT_DIR
    if pygame is None:
        return None
    ...
    for name in ("NotoSerifSC-Regular.otf", "NotoSansSC-Regular.otf", "NotoSansCJKsc-Regular.otf"):
        ...
        if font is not None:
            return attach_font_fgcolor(font, fgcolor)
    font = _sysfont_chinese(size, fgcolor)
    if font is not None:
        return font
...
def load_font_chinese_freetype(size: int, fgcolor: Tuple[int, int, int]):
    """중국어용 pygame.freetype.Font. `fgcolor`는 (R,G,B) 튜플로만 넘긴다."""
    try:
...
def load_font_korean(
    size: int,
    fgcolor: Tuple[int, int, int],
    *,
    weight: str = "regular",
) -> "Optional[pygame.font.Font]":
    """한국어용 폰트 (번역·UI·타이틀). `fgcolor`는 (R,G,B) 튜플로만 넘긴다."""
