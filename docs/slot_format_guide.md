...
- 슬롯 **제거** (`alt_word_id=0`):
  - `target_slot_order`에는 제거할 슬롯 인덱스를 넣고, 같은 위치의 `alt_word_id`에 **`0`** 을 적는다.
  - 예: `target_slot_order=2|4`, `alt_word_id=1010|0` → 2번 슬롯은 1010으로 교체, 4번 슬롯은 문장에서 **빼기**.
  - 소수 슬롯(`1.1` 등)에 `0`을 주면 그 **삽입만 생략**한다(정수 슬롯 원문은 유지).
...
## 3.1 `target_slot_order`: 정수 vs 소수 (구현 기준)

`studio/conversation/data_loading.py`에서 활용 문장을 조합할 때 다음 규칙을 따른다.

| 표기 | 의미 |
|------|------|
| **정수** (`0`, `1`, `2`, …) | `raw_sentence`의 해당 인덱스 슬롯(`{}` 순서, 0부터) **전체**를 `alt_word_id`에 대응하는 한자로 **교체**한다. |
| **소수** (`1.1`, `1.2`, `2.05`, …) | 정수 부분을 `n`이라 할 때, **n번 슬롯의 바로 뒤**(n번과 n+1번 슬롯 **사이**)에 단어를 **끼워 넣는다**. 같은 `n`에 대해 `1.1`, `1.2`처럼 소수 오름차순으로 이어 붙인다. **n번 슬롯 원문은 그대로 둔다.** |

예: `raw_sentence`가 `{这}{是}{书}`일 때

- `1.1` + 一, `1.2` + 本 → `这是一本书`（`是` 유지, 그 뒤에 一·本 삽입）
- `0` + 那 → `那是书`（0번 슬롯만 교체）

과일 문장 `{苹果}{多少}{钱}？` 에서 `0|0.1|0.2` 와 사과·一·斤 이면: 0번은 **苹果**로 두고(또는 동일 단어로 교체), `0.1`·`0.2`는 **苹果 뒤**에 붙어 `苹果一斤多少钱？` 가 된다.

**주의**: CSV에서 `1`은 정수로 파싱되고 `1.0`처럼 적어도 정수로 취급될 수 있다. 소수 슬롯에는 반드시 **소수점 이하가 있는** 표기(`1.1`)를 쓴다.
